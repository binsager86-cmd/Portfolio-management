"""
NEW TRADING SECTION - Filtered View of Transactions Table

This replaces the old ui_trading_section() that used a separate trading_history table.
Now Trading is simply a filtered view of the main transactions table.

Key changes:
1. NO data entry - all new trades must use "Add Transactions"
2. Queries from 'transactions' table (txn_type IN ('Buy', 'Sell'))
3. Edits UPDATE the 'transactions' table
4. Admin section to review/delete old trading_history records
"""

def ui_trading_section_new():
    """Trading Section - Filtered view of Buy/Sell transactions (no data entry)"""
    ...
    **📋 This is a filtered view of your Buy/Sell transactions.**
    ...
    with col1:
        start_date = st.date_input("From Date", value=date(2025, 1, 1), key="trading_start")
    with col2:
    ...
    k1.metric("🛒 Total Buys", f"{total_buys:,.0f}", f"{len(buy_df)} transactions")
    k2.metric("💰 Total Sells", f"{total_sells:,.0f}", f"{len(sell_df)} transactions")
    ...
    with col_mode:
        view_mode = st.radio("", ["📊 View", "✏️ Edit"], horizontal=True, label_visibility="collapsed")
    with col_refresh:
    ...
    if view_mode == "📊 View":
        ...
        column_config = {
            ...
            "quantity": st.column_config.NumberColumn("Quantity", min_value=0, format="%.0f"),
            ...
            try:
                ...
                for idx, row in edited_df.iterrows():
                    ...
                    if (row['date'] != orig_row['date'] or 
                        row['symbol'] != orig_row['symbol'] or
                        row['portfolio'] != orig_row['portfolio'] or
                        row['quantity'] != orig_row['quantity'] or
                        row['price'] != orig_row['price'] or
                        row['fees'] != orig_row['fees'] or
    ...
    # Admin section: Show old trading_history records for cleanup
    st.divider()
    with st.expander("🔧 Admin: Legacy Trading Data Cleanup", expanded=False):
        ...
        **Legacy Data Notice**
        ...
        portfolio calculations. Any records here do NOT affect your cash or positions.
        
        If you have test/sandbox trades from the old system, you can review and delete them below.
        """)
        ...
        try:
            ...
            if legacy_df.empty:
                ...
                with col1:
                    if st.button("🗑️ Delete ALL Legacy Records", type="secondary"):
                        try:
                            conn = get_conn()
                            cur = conn.cursor()
                ...
                with col2:
                    st.caption("This will permanently delete all trading_history records for your user. This data is NOT used in portfolio calculations anyway.")
                    
        except Exception as e:
            st.info(f"No legacy trading_history table found or error: {e}")
