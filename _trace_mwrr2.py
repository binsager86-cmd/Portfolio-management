"""Write MWRR trace to a file so it doesn't conflict with Streamlit terminal."""
...
c1 = pd.read_sql("""
    ...
    AND (is_deleted IS NULL OR is_deleted = 0) AND user_id = ?
""", conn, params=(user_id,))
...
c2 = pd.read_sql("""
    ...
    FROM transactions WHERE COALESCE(cash_dividend,0) > 0
...
out.append("\n" + "="*70)
out.append("COMPONENT 3: withdrawals (txn_type=Withdrawal/FLOW_OUT)")
out.append("="*70)
try:
    c3 = pd.read_sql("""
...
out.append("\n" + "="*70)
out.append("COMPONENT 4: ledger_deposits (txn_type=Deposit/FLOW_IN)")
out.append("="*70)
try:
    c4 = pd.read_sql("""
...
out.append("\n" + "="*70)
out.append("COMPONENT 5: transfers_in")
out.append("="*70)
try:
    c5 = pd.read_sql("""
...
out.append("\n" + "="*70)
out.append("COMPONENT 6: transfers_out")
out.append("="*70)
try:
    c6 = pd.read_sql("""
    ...
    """, conn, params=(user_id,))
except: c6 = pd.DataFrame()
out.append(f"  Rows: {len(c6)}")
if not c6.empty: out.append(c6.to_string(index=False))
...
out.append("\n" + "="*70)
out.append("COMBINED MWRR CASH FLOWS")
out.append("="*70)
...
out.append(f"\n>>> XIRR with ALL 6 components = {r*100:.4f}% <<<")
...
for _ in range(200):
    ...
    r3=rn
out.append(f">>> XIRR deposits-only (no divs) = {r3*100:.4f}% <<<")

# Print ALL signed flows for auditing
out.append("\n" + "="*70)
out.append("SIGNED CASH FLOW DETAIL (as seen by XIRR solver)")
out.append("="*70)
for dt, a in pairs:
    out.append(f"  {dt.strftime('%Y-%m-%d')}  {a:>12,.2f}")
