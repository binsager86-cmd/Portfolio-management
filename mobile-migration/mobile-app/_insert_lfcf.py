"""Insert Levered Free Cash Flow metric from financial_line_items into stock_metrics."""
import sqlite3
import time
...
# Get all LEVERED_FREE_CASH_FLOW line items
rows = db.execute(
...
for stock_id, fiscal_year, period_end_date, amount in rows:
    ...
    existing = db.execute(
        ...
        (stock_id, "Levered Free Cash Flow", period_end_date),
    ).fetchone()
    if existing:
        db.execute(
            "UPDATE stock_metrics SET metric_value = ?, created_at = ? WHERE id = ?",
            (amount, now, existing[0]),
        )
        ...
        db.execute(
            """INSERT INTO stock_metrics
               ...
               VALUES (?,?,?,?,?,?,?,?)""",
            (stock_id, fiscal_year, None, period_end_date, "cashflow", "Levered Free Cash Flow", amount, now),
        )
