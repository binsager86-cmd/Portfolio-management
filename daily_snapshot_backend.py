...
def require_columns(con: sqlite3.Connection, table: str, required: List[str]) -> None:
    ...
    if missing:
        raise RuntimeError(f"Table '{table}' missing columns {missing}. Existing columns: {cols}")
...
def detect_ledger_schema(con: sqlite3.Connection) -> Dict[str, str]:
    require_columns(con, LEDGER_TABLE, [])  # must exist

    cols = get_cols(con, LEDGER_TABLE)

    asset_col = pick_col(cols, ["asset_id", "assetid"])

    # ✅ add your real column name entry_datetime
    date_col = pick_col(cols, ["entry_datetime", "txn_date", "trade_date", "date", "entry_date"])

    # ✅ add your real column name entry_type
    type_col = pick_col(cols, ["entry_type", "txn_type", "type", "transaction_type"])

    qty_col = pick_col(cols, ["quantity", "qty", "shares"])
    price_col = pick_col(cols, ["price", "unit_price", "trade_price"])  # optional
